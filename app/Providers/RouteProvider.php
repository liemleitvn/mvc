    ...
    public function dispatch() {
        ...
        try {
            ...
            if(is_string($matcher['_controller']) && strpos($matcher['_controller'], '::') !== false) {
                ...
                unset($matcher['_controller']);
                unset($matcher['_route']);

                $queryStringSplitter = explode('?', $matcher[count($matcher)-1]);
                $matcher[count($matcher)-1] = $queryStringSplitter[0];

                array_unshift($matcher, new Request());
